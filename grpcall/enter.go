...
func SetProtoContents(importPath, fileName, protoContent string) {
	descSourceController.SetProtoContent(importPath, fileName, protoContent)
}
...
func (d *DescSourceEntry) SetProtoContent(importPath string, protoFile string, content string) {
	d.importPaths.Set(importPath)
	d.protoFiles.Set(protoFile)
	d.protoContent = content
}
...
func (d *DescSourceEntry) InitDescSource() error {
	...
	case ProtoContentMode:
		descSourceController.descSource, err = DescriptorSourceFromProtoContent(
			descSourceController.importPaths,
			descSourceController.protoFiles[0],
			map[string]string{
				descSourceController.importPaths[0] + "/" + descSourceController.protoFiles[0]: descSourceController.protoContent,
			},
		)
		if err != nil {
			return errors.New("failed to process proto content")
		}
...
func (e *EngineHandler) CallWithCtx(ctx context.Context, target, serviceName, methodName, data string, gclient *grpc.ClientConn) (*ResultModel, error) {
	return e.invokeCall(ctx, gclient, target, serviceName, methodName, data)
}

func (e *EngineHandler) Call(target, serviceName, methodName, data string, gclient *grpc.ClientConn) (*ResultModel, error) {
	return e.invokeCall(e.ctx, gclient, target, serviceName, methodName, data)
}
