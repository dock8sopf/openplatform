...
func DescriptorSourceFromProtoContent(importPaths []string, fileNames string, contentMap map[string]string) (DescriptorSource, error) {
	p := protoparse.Parser{
		ImportPaths:      importPaths,
		InferImportPaths: len(importPaths) == 0,
		Accessor:         protoparse.FileContentsFromMap(contentMap),
	}
	fds, err := p.ParseFiles(fileNames)
	if err != nil {
		return nil, fmt.Errorf("could not parse given files: %v", err)
